...
public class CustomFile extends CalDavFileBase {
	...
	public CustomFile(FileItem item, CalDavResourceLocator calDavResourceLocator,
	...
	public void writeTo(OutputContext outputContext) throws CosmoDavException, IOException {
		...
		String contentType = ContentTypeUtil.buildContentType(content.getContentType(), content.getContentEncoding());
		...
		long len = content.getContentLength() != null ? content.getContentLength().longValue() : 0;
		...
		FileCopyUtils.copy(content.getContentInputStream(), outputContext.getOutputStream());
	}
